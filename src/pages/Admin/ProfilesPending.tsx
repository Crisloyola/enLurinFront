...
export default function ProfilesPending() {
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message ?? err?.response?.data ?? 'Error al aprobar'
      setErrors(prev => ({ ...prev, [id]: typeof msg === 'string' ? msg : 'Error al aprobar' }))
    } finally {
      setProcessing(null)
    }
  }
  ...
  const handleSuspend = async (id: number) => {
    ...
    try {
      ...
      setErrors(prev => ({ ...prev, [id]: 'Error al suspender' }))
    } finally {
      setProcessing(null)
    }
  }

  if (isLoading) return <Loader text="Cargando pendientes..." />

  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <h1 className="font-extrabold text-2xl text-gray-900 mb-1">Perfiles Pendientes</h1>
      <p className="text-gray-500 text-sm mb-8">{profiles?.length ?? 0} perfil(es) esperando aprobación</p>
            ...
            <div key={p.id} className="bg-white rounded-2xl shadow-sm p-5">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-orange-600 font-semibold mb-1">{p.category} — {p.district}</p>
              ...
              {errors[p.id] && (
                <p className="text-red-500 text-xs mt-2 font-medium">{errors[p.id]}</p>
              )}
            </div>
          ))}
        </div>
